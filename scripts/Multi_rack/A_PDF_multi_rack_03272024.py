...
def useful_info(): # .......Captures metadata   
	info_dict = {}
	...
	info_dict['OT_stage_X'] = OT_stage_2_X.read()
	info_dict['OT_stage_Y'] = OT_stage_2_Y.read()
	info_dict['Spinnergo_Y'] = Spinnergo_Y.read()
	info_dict['broadside45'] = broadside45_shifter.read()
	info_dict['ring_current'] = ring_current.read()
	info_dict['frame_acq_time'] = glbl['frame_acq_time']
	info_dict['dk_window'] = glbl['dk_window']
	info_dict['Measurement_time'] = time.time()
...
Ex) xrun(sample index, scanplan, more_info = useful_info())
    ...
    for posy, smpl_list in zip(posylist, smplist):
        ...
        for num in range (num_PDF):
            for idx in range (len (posxlist)):
                ...
                xrun(smpl_list[idx], ScanPlan_PDF, more_info = useful_info())
                print('sleep')
...
D1 = 2906   # PDF position of Det_1_Z position
D2 = xxxxx  # XRD position of Det_1_Z position
...
#smpl_list_PDF_2=[104,105,106,107,108,109]  #PDF   #####
...
''' 
RE(mv(OT_stage_2_Y, pos_list_y1))
for num in range (num_PDF):
    for idx in range (len (pos_list_x)):
        print('moving OT_stage_2_X', pos_list_x[idx])
        RE(mv(OT_stage_2_X, pos_list_x[idx]))
        print(smpl_list_PDF_1[idx])

        xrun(smpl_list_PDF_1[idx], ScanPlan_PDF, more_info = measurement_data())
        print('sleep')
        glbl['frame_acq_time']=0.1
        time.sleep(90)    ### 90
        glbl['frame_acq_time']=det_exp_PDF   ######## 0.5
...
RE(mv(OT_stage_2_Y, pos_list_y2))
for num in range (num_PDF):
    for idx in range (len (pos_list_x)):
        print('moving OT_stage_2_X', pos_list_x[idx])
        RE(mv(OT_stage_2_X, pos_list_x[idx]))
        print(smpl_list_PDF_2[idx])
    
        xrun(smpl_list_PDF_2[idx], ScanPlan_PDF, more_info = measurement_data())
        print('sleep')
        glbl['frame_acq_time']=0.1
        time.sleep(90)    ### 90
        glbl['frame_acq_time']=det_exp_PDF   ######## 0.5
